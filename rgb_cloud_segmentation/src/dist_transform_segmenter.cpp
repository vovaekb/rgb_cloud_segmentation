...
DistTransformSegmenter::DistTransformSegmenter(int dist_transform_mask_size, bool visualize) : dist_transform_mask_size_(dist_transform_mask_size),
...
void DistTransformSegmenter::segment()
{
    ...
    cv::threshold(bw, bw, 100, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
    cv::imshow("Binary Image", bw);

    // Perform the distance transform algorithm
    cv::Mat dist;
    cv::distanceTransform(bw, dist, CV_DIST_L2, dist_transform_mask_size_);
...
std::vector<points_vector> DistTransformSegmenter::getAreas()
{
    return areas_;
}
