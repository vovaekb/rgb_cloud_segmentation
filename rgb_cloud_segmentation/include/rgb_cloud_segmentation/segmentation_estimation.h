...
private:
    ...
    struct Segment
    {
        ...
        std::pair<Label, size_t> getLargestOverlap () const
        {
          Label label;
          size_t max = 0;
          typedef std::map<Label, std::vector<size_t> >::const_iterator it_type;
...
//          }
    ...
    void computeOverlap ()
    {
      ...
      typedef std::map<Label, Segment>::iterator it_type;
...
//          {
              ...
              typedef std::map<Label, std::vector<size_t> >::iterator it_type;
...
//          }
    ...
    void computeCovering ()
    {
        ...
        float covering = 0.0;
        float points_n = (float)groundtruth_cloud_.size();
        typedef std::map<Label, Segment>::iterator it_type;
        ...
        for(it_type ls_pair = overlap_map_.begin(); ls_pair != overlap_map_.end(); ls_pair++)
        {
            ...
            Segment segment = ls_pair->second;
            int segment_size = (int)segment.size;
            ...
            for(it_type li_pair = segment_overlaps.begin(); li_pair != segment_overlaps.end(); li_pair++)
            {
                ...
                PCL_INFO("Overlap with %d (%d points): %d points\n", (int) label, (int) overlap_segment_size, (int) overlap_size);

                int overlap_denominator = segment_size + overlap_segment_size - overlap_size;

                PCL_INFO("Size of union of segments (%d and %d): %d\n", segment_size, (int) overlap_segment_size, overlap_denominator);

                float overlap = (float) overlap_size / (float) overlap_denominator;
...
//            std::pair<Label, size_t> largest_overlap = segment.getLargestOverlap();
//            Label overlap_label = largest_overlap.first;
//            int overlap_size = (int)largest_overlap.second;

//            int overlap_segment_size = getOverlapSegmentSize(overlap_label);

//            int overlap_denominator = segment_size + overlap_segment_size - overlap_size;

//            float max_overlap = (float)overlap_size / (float)overlap_denominator;

            float segment_covering = segment_size * max_overlap;

            covering += segment_covering;

            PCL_INFO("Segment %d covering: %.3f\n\n", (int)ls_pair->first, segment_covering);
        }
